...
Options:
    ...
    -r  --range      : String - Start and End week, separated by a dot <.> (-r 1.10)
...
def processTheClass(schoolClass: bs):
    ...
    try:
        ...
        eduNextUrl = (
            schoolClass.find("a", {"class", "label label-primary"})["href"]
            if schoolClass.find("a", {"class", "label label-primary"})
            else None
        )
        materialUrl = (
            schoolClass.find("a", {"class", "label label-warning"})["href"]
            if schoolClass.find("a", {"class", "label label-warning"})
            else None
        )
        room = re.search(pattern, schoolClass.text).group().replace("at", "").strip()
        isOnline = (
            True if schoolClass.find("div", {"class": "online-indicator"}) else False
        )
        meetUrl = (
            schoolClass.find("a", {"class": "label label-default"})["href"]
            if schoolClass.find("a", {"class": "label label-default"})
            else None
        )
        return {
            "name": name,
...
def turnTheTableIntoJSON(soup: bs):
    ...
    dates = [
        date.text
        for date in soup.find("div", {"id": "ctl00_mainContent_divShowDate"}).find_all(
            "th"
        )
    ]
    everySlots = [
        slot
        for slot in soup.find("div", {"id": "ctl00_mainContent_divContent"}).find_all(
            "tr"
        )
    ]
    year = (
        soup.find("select", {"name": "ctl00$mainContent$drpYear"})
...
def getScheduleData(cookies, headers, check=False):
    ...
    selectThatHasWeeks = soup.find(
        "select", {"name": "ctl00$mainContent$drpSelectWeek"}
    )
    (years, yearsDict, selectedYear) = processTheSelectIntoArrayDictSelectedValue(
        selectThatHasYears
    )
    (weeks, weeksDict, selectedWeek) = processTheSelectIntoArrayDictSelectedValue(
        selectThatHasWeeks
    )
    ...
    return (
        soup,
        (years, yearsDict, selectedYear),
        (weeks, weeksDict, selectedWeek),
        (__VIEWSTATE, __EVENTVALIDATION),
    )


def getScheduleDataWithYearAndWeek(
    cookies, headers, year, week, aspnetFormData, check=False
):
    data = {
...
def getScheduleDataWithYearAndWeeks(cookies, headers, year, weeks, aspnetFormData):
    ...
    for week in weeks:
        ...
        soups.append(
            getScheduleDataWithYearAndWeek(
                cookies=cookies,
                headers=headers,
                year=year,
                week=week,
                aspnetFormData=aspnetFormData,
                check=False,
            )
        )
    return soups
...
def getScheduleFromWeekRange(cookies, headers, year, wRange, aspnetFormData):
    start, end = map(int, wRange.split("."))
    soups = []
    ...
    if all([year, weeks, week]):
        ...
        if year == yearData[2] and week == weekData[2]:
            ...
            if week:
                if "." in week:
                    raise Exception(
                        "Option -w or --week must be an Integer. Maybe you want to use --weeks"
                    )
                soup = getScheduleDataWithYearAndWeek(
                    cookies=cookies,
                    headers=headers,
                    year=year,
                    week=week,
                ...
                soup = getScheduleDataWithYearAndWeeks(
                    cookies=cookies,
                    headers=headers,
                    year=year,
                    weeks=weeks,
